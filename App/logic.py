...
def add_crime(analyzer, crime):
    ...
    update_area_index(analyzer['areaIndex'], crime)  # Actualizamos el índice por áreas
...
def get_crimes_by_range_area(analyzer, initialArea, finalArea):
    ...
    """
    lst = rbt.keys(analyzer["areaIndex"], initialArea, finalArea)
    totalcrimes = 0
    for area in lst["elements"]:
        # Por cada área, sumamos los crímenes asociados a esa área
        totalcrimes += al.size(area["lstoffenses"])
    return totalcrimes
